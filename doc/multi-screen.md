...
This sim was scaffolded by `npm run scaffold-screens` into `src/introduction/`, `src/dielectric/`, `src/multiple-capacitors/` (fleet naming:
...
1. Copies the `introduction/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `introduction/` folder
...
main.ts
  └─ IntroductionScreen            (Screen<IntroductionModel, IntroductionScreenView>)
       ├─ IntroductionModel         owns all state
       └─ IntroductionScreenView    owns all visuals
```
...
template's `IntroductionScreen.ts`. Screen icons live in one shared module under
...
```
src/common/CapacitorLabScreenIcons.ts
```
