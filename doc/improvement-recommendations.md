# Improvement Recommendations — Capacitor Lab
...
prefixes (`Sim` → `CapacitorLabColors` → `CapacitorLabConstants`). It works today only because the
bare token `Sim` is not in the list, but adding it (a common request) would
silently corrupt every `CapacitorLabColors`/`CapacitorLabConstants` occurrence. Either:
...
- `getIntroductionA11yStrings()` and `getPreferences()` return inferred JSON types.
...
### 8. No dispose-pattern reference in `IntroductionScreenView`

`src/introduction/view/IntroductionScreenView.ts` is billed (in `CLAUDE.md`) as the
...
### 14. `IntroductionKeyboardHelpContent` ships only `BasicActionsKeyboardHelpSection`

`src/introduction/view/IntroductionKeyboardHelpContent.ts` constructs
