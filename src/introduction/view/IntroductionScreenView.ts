...
export type IntroductionScreenViewOptions = ScreenViewOptions;

export class IntroductionScreenView extends ScreenView {
  public constructor(model: IntroductionModel, providedOptions?: IntroductionScreenViewOptions) {
    ...
    // it the same way. See IntroductionScreenSummaryContent for the four content regions.
    const options = optionize<IntroductionScreenViewOptions, EmptySelfOptions, ScreenViewOptions>()(
      {
    ...
    const placeholderText = new Text(StringManager.getInstance().getScreenNames().introductionStringProperty, {
  ...
  //
  // private readonly disposeIntroductionScreenView: () => void;
  //
  ...
  //   [model.stateProperty],
  //   (state) => `State: ${state}`,
  // ); // → pass as currentDetailsContent in IntroductionScreenSummaryContent
  // const onExternal = (v: number) => { /* … */ };
  // model.externalProperty.link(onExternal); // Property you do NOT own
  //
  // this.disposeIntroductionScreenView = () => {
